...
import com.prussian_trooper.project.boardtable.databinding.SignDialogBinding
...
class DialogHelper(act: MainActivity) {
    ...
    fun createSignDialog(index: Int) {
        ...
        val view =
            rootDialogElement.root //rootDialogElement превращается во view(которая стоит после val)
    ...
    private fun setOnClickSignUpIn(
        ...
        if (index == DialogConst.SIGN_UP_STATE) {
            ...
            accHelper.signUpWithEmail(
                rootDialogElement.edSignEmail.text.toString(),//Регистрация аккаунта
                rootDialogElement.edSignPassword.text.toString()
            )

        } else {
            accHelper.signInWithEmail(
                rootDialogElement.edSignEmail.text.toString(),//Вход по паролю
                rootDialogElement.edSignPassword.text.toString()
            )
        }
    }
